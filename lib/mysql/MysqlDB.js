...
function MysqlDB() {
    ...
    this.insertBatch = async function (table, models, delayed) {
        var values = [];
        var keys = null;
        for (var i = 0; i < models.length; i++) {
            ...
            var value = _.values(model);
            values.push(value);
        }
        var sql = 'insert' + (delayed ? ' delayed ' : ' ') + 'into ?? (??) values ?';
        var result = await this.query(sql, [table,keys,values]);
    ...
    this.exists = async function(table, conditions) {
        conditions = conditions || {};
        conditions.where = conditions.where || "1=1";
        table = "`" + table + "`";
        var sql = `select 1 from ${table} where ${conditions.where} `;
        var rows = await this.query(sql, conditions.params);
        if (rows.length > 0) {
            return true;
        }

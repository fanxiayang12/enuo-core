...
function MongoDB() {
    ...
    this.findAll = async function (collectionName, query, options) {
        if (this.debug) {
            logger.info("findAll");
            logger.info("collection:", collectionName);
            logger.info("query:", query);
        ...
        var collection = this.db.collection(collectionName);
        var resultList = [];
        var docs = collection.find(query, { timeout: false });

        let count = await docs.count();

        logger.info("docs.count..", count);
        // logger.info("docs:123",docs);

        var step = 1000;
        var page = parseInt(count / step);
        var temp = count % step;
        if (temp > 0) {
            page += 1;
        }
        ...
        for (let i = 0; i < page; i++) {
            var tempRsult = collection.find(query).skip(i * step).limit(step);
            let tempArray = await tempRsult.toArray();
            resultList = resultList.concat(tempArray);
            logger.info(i + ":tempResult.length:", tempArray.length);
            await tempRsult.close();
        }
        logger.info("resultList:", resultList.length);
        logger.info("time-consuming:", (new Date().getTime() - nowTime) / 1000);
        return resultList;
        ...
        // while( docs.hasNext()){
        //     console.log("index..",i++);
        //     var doc = await docs.next();
        //     resultList.push(docs);
        //   };

        // // var tempList = await docs.toArray();
    ...
    this.aggregate = async function (collectionName, pipeline, options) {
        if (!options) {
            options = {};
        }
        var collection = this.db.collection(collectionName);
    ...
    this.save = async function (collectionName, doc, options) {
        var _id = doc._id ? new ObjectID(doc._id) : new ObjectID();
